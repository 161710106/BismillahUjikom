...
<div class="col-xs-12">
          <div class="box">
            ...
            <center><h3 class="box-title">Edit Data Barang</h3></center>
        <form action="{{ route('barangs.update',$barangs->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('nama_barang') ? ' has-error' : '' }}">
                      ...
                      <div class="form-group {{ $errors->has('kategoribarang_id') ? ' has-error' : '' }}">
			  			<label class="control-label">Kategori</label>	
			  			<select name="kategoribarang_id" class="form-control">
			  				@foreach($kategoris as $data)
			  				<option value="{{ $data->id }}" {{ $selectedkategoribarang == $data->id ? 'selected="selected"' : '' }} >{{ $data->kategori }}</option>
			  				@endforeach
			  			</select>
			  			@if ($errors->has('kategoribarang_id'))
                            <span class="help-block">
                                <strong>{{ $errors->first('kategoribarang_id') }}</strong>
                            </span>
                        @endif
					  </div>
